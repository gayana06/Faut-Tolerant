...
public class ProcessSet implements Serializable
{
   ...
   */
	 ...
	 */
	public UniqueProcess[] getAllProcesses()
	{
		return processes;
	}
	 ...
	 */
	public void addProcess(UniqueProcess process, int pr)
	{
		if (pr >= processes.length)
		{
			UniqueProcess[] temp = new UniqueProcess[processes.length + 1];
			for (int i = 0; i < processes.length; i++)
				temp[i] = processes[i];
			processes = temp;
		}
	 ...
	 * @return the process
	 */
	public UniqueProcess getProcess(int i)
	{
		return processes[i];
	}
	 ...
	 * @return the process.
	 */
	public UniqueProcess getProcess(SocketAddress addr)
	{
		int i = getRank(addr);
		if (i == -1)
			return null;
	 ...
	 * @return My process
	 */
	public UniqueProcess getSelfProcess()
	{
	...
	public ProcessSet cloneProcessSet()
	{
		...
		UniqueProcess[] procs = getAllProcesses();
